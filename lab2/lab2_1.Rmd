...
```{r}
z=10 #this will wor, but will create problems
```
...
treatment <-  36
control <-  38
#run the code before you caluclate the value
```
...
```{r}
mean(2,8,2) #does not use vector
```
...
```{r}
my_integer <- as.integer(my_numeric)#will be important when working with plots
```
