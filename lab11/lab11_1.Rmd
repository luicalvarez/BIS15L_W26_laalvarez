...
gap %>% 
  ...
  geom_line()+
  geom_point(shape=5)+
  theme(scale_x_continuous(breaks = 2000:2025,minor_breaks=NULL))+
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_histogram(color="black",fill="blue4", bins=30)
```
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_density(color="black",fill="red4", alpha=.7)
```
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_histogram(aes(y=after_stat(density)), color="black", fill="blue4")+
...
income_cat2000 %>% 
  ...
  select(country, income_category) %>% 
  ggplot(aes(x=income_category))+
  geom_bar(aes(fill = income_category))
```
