...
```{r}
ggplot(data = penguins,mapping = aes(x=body_mass_g, y=flipper_length_mm))+geom_point(na.rm = T)+geom_smooth(na.rm=T,method = lm, se=T)
```
...
```{r}
ggplot(data = penguins,mapping = aes(x=body_mass_g, y=flipper_length_mm, colour = species))+geom_point(na.rm = T)+geom_smooth(na.rm=T,method = lm, se=T)
```
...
```{r}
ggplot(data = penguins,mapping = aes(x=body_mass_g, y=flipper_length_mm))+geom_point(na.rm = T)+geom_point(mapping = aes(color= species))+geom_smooth(na.rm=T,method = lm, se=T)
```
...
```{r}
ggplot(data = penguins,mapping = aes(x=body_mass_g, y=flipper_length_mm))+geom_point(na.rm = T)+geom_point(mapping = aes(color= species,shape = species))+geom_smooth(method = lm, se=T)
```

But don't we want a title? We do this using the `labs()` function.
```{r}
ggplot(data = penguins,mapping = aes(x=body_mass_g, y=flipper_length_mm))+geom_point(na.rm = T)+geom_point(mapping = aes(color= species,shape = species))+geom_smooth(method = lm, se=T)+labs(title = "Body Mass (g) vs Flipper Length (mm)")
```

### Practice
1. Is there a relationship between bill length and bill depth?
```{r}
ggplot(data = penguins,mapping = aes(x=bill_length_mm, y=bill_depth_mm))+geom_point(mapping = aes(color=species, shape=species))+geom_smooth(method = lm, se=F)+labs(title = "Bill Length (mm) vs Bill Length Depth (mm)")
```
...
ggplot(data=penguins,
  ...
  labs(title = "Number of Penguins Sampled per Island Surveyed")      
```
