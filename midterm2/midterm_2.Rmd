...
polar_bears %>% 
  ...
  geom_col()+
  labs(title = "Number of Sampled Polar Bears Divided By Sex",
       x= "Sex",
       y= "Polar Bear Count")
```
...
```{r}
polar_bears %>% 
  select(capture_date,sex) %>% 
  separate(capture_date, into = c("year", "date"))
#Study how to create new columns next time
```
```{r}
#polar_bears %>% 
#ggplot(aes(x=year, y=count))+
  #geom_line()+
  #geom_point(shape=5)+
  #theme(scale_x_continuous(breaks = 2985:2017,minor_breaks=NULL))+
...
polar_bears %>% 
  select(urea_nitrogen, creatinine) %>% 
  summarise(un_creat_ratio=urea_nitrogen*.466/creatinine)
#Study new column making cant do rest
```
...
#polar_bears %>% 
  ...
  #case_when(fasting,un_creat_ratio <= 10,
            #not_fasting, un_creat_ratio >= 10 )
#Dont have the new column
```
...
#polar_bears %>% 
  ...
  #Dont have the variables study how to make new variables
```
...
```{r}
ggmap(map1)+
  geom_point(data = polar_bears,aes(long,lat), size=.75,color="red4", alpha=.8)+
...
```{r, eval=F, echo=T}
library(shiny) #None of the made column wont work, study
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    polar_bears %>% 
      ...
      labs(title="ure_nitrogen/creatine ratio by Population Type",
           x="Population Type",
