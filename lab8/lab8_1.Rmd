...
elephants %>%
  ...
  summarise(mean_estimated_age_years=mean(estimated_age_years, na.rm=T), n=n(), .groups = ='keep') %>%
  arrange(sex)
```
...
elephants %>%
  ...
  group_by(years_of_sample_collection, sex) %>%
  summarise(average_age=mean(estimated_age_years, na.rm=T), median_age=median(estimated_age_years, na.rm=T), sd_age=sd(estimated_age_years, na.rm=T), n=n()) %>%
  arrange(sex)
```
...
msleep %>%
  group_by(vore) %>%
  summarise(min_bt=min(bodywt, na.rm=T), max_bt=max(bodywt, na.rm=T), mean_bt=mean(bodywt, na.rm=T), n=n())
```
...
msleep %>%
  group_by(order) %>%
  summarise(mean_bw=mean(brainwt, na.rm=T), n=n())
```
...
msleep %>%
  group_by(order) %>%
  summarise(mean_bw=mean(brainwt, na.rm=TRUE), n=n())
```
