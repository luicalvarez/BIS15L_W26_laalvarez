...
```{r}
p+geom_jitter(size=1.8)
```

We can color the points by a categorical variable.
```{r}
p+geom_point(aes(color=thermoregulation),size=1.8)
```

We can also map shapes to another categorical variable.
```{r}
p+geom_point(aes(shape=thermoregulation,color=thermoregulation),size=1.8)
```

What about just filling in with a color preference or size?
```{r}
p+geom_point(size=3, color="black", alpha=0.5,shape=22,fill="springgreen")
```
...
```{r}
homerange %>%
  ggplot(mapping = aes(x=taxon,fill=trophic_guild))+
  geom_bar(position = "dodge")+
  theme(axis.txt.x=element_text(angle=60,hjust = .85))+
  coord_flip()+
  labs(title = "Observations By Taxon",
       x="Taxonomic Group",
       y=NULL)
