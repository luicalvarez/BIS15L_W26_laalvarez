...
```{r}
sydbeaches <- read_csv("data/sydneybeaches.csv")

```

2. Are these data "tidy" per the definitions of the tidyverse? How do you know? Are they in wide or long format?
```{r}
#The data is not tidy as the data should be moved to a long format so that points like Site or Region are not considered data points.
```

3. We are only interested in the variables site, date, and enterococci_cfu_100ml. Make a new object focused on these variables only. Name the object `sydneybeaches_long`
```{r}
sydbeaches_long <- sydbeaches %>% 
...
```{r}
sydbeaches_wide <- sydbeaches_long %>% 
...
```{r}
sydbeaches_long2 <- sydbeaches_long %>% 
...
sydbeaches_long2 %>% 
  group_by(year,Site) %>% 
  select(`Enterococci (cfu/100ml)`) %>% 
  summarise(mean_en=mean(`Enterococci (cfu/100ml)`))
```


8. Make the output from question 7 easier to read by pivoting it to wide format.
```{r}
sydbeaches_wide2 <- sydbeaches_long2 %>% 
  group_by(year,Site) %>% 
  select(`Enterococci (cfu/100ml)`) %>% 
  summarise(mean_en=mean(`Enterococci (cfu/100ml)`)) %>% 
...
sydbeaches_wide2 %>% 
  filter(year=="2013")
  #The most polutted was little bay beach
```

10. Explore the data! Do one analysis of your choice that includes a minimum of three lines of code.  
```{r}
sydbeaches_wide2 %>% 
  ggplot(mapping = aes(x= `Bondi Beach`, y=year))+
  geom_col(mapping = aes(fill = year)) +
  labs(title = "Pollution Reccorded in Bondi Beach Between 2013 and 2017")
#The most pollution recorded in bondi beach was in 2013
```
