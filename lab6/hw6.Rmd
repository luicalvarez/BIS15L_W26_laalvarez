...
```{r}
glimpse(gabon_clean_f)
```

```{r}
gabon_clean_f %>%
  select("land_use", "transect_id") %>%
ggplot(data = gabon_clean_f,
...
gabon_clean_f %>%
  ggplot(data = gabon_clean_f,
         mapping = aes(x=rich_mammal_species, y=veg_rich))+
  geom_point(mapping = aes(colour = land_use))+
  geom_smooth(method = lm, se=F)
#There does not seem to be much of a correlation
```
...
names(gabon_clean_f)
gabon_clean_f %>%
  select("transect_id", "veg_rich", "veg_stems") %>%
  filter("veg_rich" >=15 | "veg_stems" %in% c(50,60))
```
...
```{r}
gabon_clean_f %>%
  select("transect_id", "veg_rich", "diversity_mammal_species", "land_use") %>%
  filter("veg_rich" >=10 & "diversity_mammal_species" >= 10) %>%
