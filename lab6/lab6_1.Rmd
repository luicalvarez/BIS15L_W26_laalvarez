...
ggplot(data = penguins,
  ...
  labs(title = "Mean body mass (g) vs mean flipper length (mm)")
```
...
```{r}
penguins %>%
  distinct(island, species, .keep_all=T) #keep all keeps all the other variables for 1 of matches
```
...
```{r}
mammals %>%
  mutate(across(everything(),tolower)) #be careful with numaric variables
```
...
```{r}
ggplot(data = mammals,
       mapping = aes(x=log10(newborn), y=gestation))+
  geom_point()
```
...
```{r}
ggplot(data = mammals,
       mapping = aes(x=max_life, y=log10(mass)))+
  geom_point()
```
