...
msleep %>%
  filter(bodywt > 150) %>%
  summarise(mean_bodywt_l=mean(bodywt))
```
...
msleep %>%
  filter(bodywt > 150) %>%
  summarise(mean_bodywt_l=mean(bodywt), min_bodywt_l=min(bodywt),max_bodywt_l=max(bodywt), sd_bodywt_l=sd(bodywt), total=n())
```
...
msleep %>%
  ...
  summarise(mean_bodywt_p=mean(bodywt), min_bodywt_p=min(bodywt),max_bodywt=max(bodywt), total=n())
```
...
msleep %>%
  ...
  geom_col() #use this when we have an x and y not just for counts
```
